...
const prompt = ChatPromptTemplate.fromMessages([
  ['system', 'You are a helpful assistant for an app called Nom. Your role is to generate or modify meal plans for the user. By default, meal plans contain 3 meals a day for a week. When creating a meal plan, start it at the next upcoming meal based on the time and day you receive the request. You must output the meal plan in the following JSON format regardless of the context: {{"days":[{{"day":1,"meals":{{"breakfast":"","lunch":"","dinner":""}}}}, ... ]}}'],
  ['user', '{input}'],
...
async function parse_and_verify_output(output) {
  try {
    ...
    // commented out Edamam API verification for now
    // for (const day of meal_plan.days) {
    //   for (const [meal_type, meal] of Object.entries(day.meals)) {
    //     if (meal) {
    //       const recipes = await query_edamam_api.func(meal);
    //       console.log(`Recipes for ${meal_type} (${meal}):`, recipes);
    //     }
    //   }
    // }
